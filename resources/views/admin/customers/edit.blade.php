    ...
    <div class="create-wrap"
        ...
        <div class="create-header" style="display: flex; align-items: center; gap: 15px; margin-bottom: 25px;">
            <div class="create-header-icon"
                style="background: #f59e0b; width:45px; height:45px; border-radius:10px; display:flex; align-items:center; justify-content:center; color:white; font-size:18px;">
                <i class="fa-solid fa-user-pen"></i>
            </div>
            <div>
                <h2 class="create-header-title" style="margin:0; font-size:18px; font-weight:800; color: #1e293b;">Modifier le
                    client : {{ $customer->name }}</h2>
                <p class="create-header-sub" style="margin:0; font-size:12px; color: var(--text-muted);">Mettez à jour les
                    informations et soldes du client</p>
            </div>
        </div>

        <form id="customer-form" method="POST" action="{{ route('admin.customers.update', $customer->id) }}">
            @csrf
            @method('PUT')
            ...
            <div
                ...
                <button type="submit" class="btn btn-yellow">Sauvegarder les modifications</button>
            </div>
        </form>
    </div>
