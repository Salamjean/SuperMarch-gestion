...
@section('page-title', 'Modifier categorie')
    ...
    <div class="cat-create-shell">
        <section class="cat-create-hero">
            <div class="cat-hero-left">
                ...
                <h2>Mettre a jour une categorie</h2>
                <p>Modifiez le nom, la description, la couleur et le statut pour garder un classement coherent des produits.
                </p>
            </div>
            <div class="cat-hero-preview" id="heroPreview">
                <span class="cat-chip" id="previewChip">
                    <i class="fa-solid fa-tag"></i>
                    <span id="previewName">{{ old('name', $category->name) }}</span>
                </span>
                <small id="previewState">Statut: {{ old('is_active', $category->is_active) ? 'Actif' : 'Inactif' }}</small>
            </div>
        </section>

        <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="cat-form-grid"
            autocomplete="off">
            @csrf
            @method('PUT')
                ...
                <div class="cat-field">
                    ...
                    <div class="cat-input-wrap cat-textarea-wrap {{ $errors->has('description') ? 'is-invalid' : '' }}">
                        ...
                        <textarea id="description" name="description" rows="5" maxlength="500"
                            placeholder="Description de la categorie (facultatif)">{{ old('description', $category->description) }}</textarea>
                    </div>
                    @error('description')
                        <p class="cat-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</p>
                    @enderror
            ...
            <article class="cat-form-panel">
                ...
                <div class="cat-field">
                    ...
                    <div class="cat-color-wrap {{ $errors->has('color') ? 'is-invalid' : '' }}">
                        ...
                        <div>
                            <p class="cat-color-code" id="colorCode">
                                {{ strtoupper(old('color', $category->color ?? '#004d99')) }}</p>
                ...
                <div class="cat-field">
                    ...
                    <label class="cat-toggle">
                        ...
                        <span class="cat-toggle-label"
                            id="toggleLabel">{{ old('is_active', $category->is_active) ? 'Categorie active' : 'Categorie inactive' }}</span>
                    </label>
                </div>
...
@push('scripts')
    <script>
        (function() {
            ...
            const syncPreview = () => {
                const label = (nameInput.value || 'Categorie').trim();
