        ...
        <table id="stock-table" style="width: 100%; border-collapse: collapse; text-align: center;">
            <thead>
                <tr style="background: #f8fafc; border-bottom: 1px solid var(--border);">
                    <th
                        style="padding: 15px 20px; font-size: 13px; color: var(--text-muted); text-transform: uppercase;">
                        Aperçu</th>
                    <th
                        style="padding: 15px 20px; font-size: 13px; color: var(--text-muted); text-transform: uppercase;">
                        Produit</th>
            ...
            <tbody>
                @foreach ($allProducts as $product)
                    <tr class="stock-row" data-product-id="{{ $product->id }}"
                        data-name="{{ strtolower($product->name) }}" data-category="{{ $product->category_name }}"
                        data-is-low="{{ $product->stock <= $product->stock_threshold ? 'true' : 'false' }}"
                        style="border-bottom: 1px solid #f1f5f9;">
                        <td style="padding: 10px 20px;">
                            <div
                                style="width: 45px; height: 45px; border-radius: 8px; overflow: hidden; background: #f1f5f9; display: inline-flex; align-items: center; justify-content: center;">
                                @if ($product->image)
                                    <img src="{{ asset('storage/' . $product->image) }}"
                                        style="width: 100%; height: 100%; object-fit: cover;">
                                @else
                                    <div
                                        style="width:100%; height:100%; background: linear-gradient(135deg, #004d99, #1a6bbf); display:flex; align-items:center; justify-content:center; color:white; font-weight:800; font-size:18px;">
                        ...
                        <td style="padding: 15px 20px; font-size: 14px; font-weight: 700;">
                            {{ number_format($product->price, 0, ',', ' ') }} FCFA</td>
                        <td style="padding: 15px 20px;" id="stock-val-{{ $product->id }}">
                            @if ($product->stock <= $product->stock_threshold)
                                <span
                                    style="background: #fff1f2; color: #e11d48; padding: 4px 10px; border-radius: 6px; font-size: 13px; font-weight: 700;">
                                    {{ $product->stock }} (Bas)
                                </span>
                            @else
                                <span
                                    style="background: #e8f9f0; color: #059669; padding: 4px 10px; border-radius: 6px; font-size: 13px; font-weight: 700;">
                                    {{ $product->stock }}
                                </span>
                            @endif
                        </td>
                        <td style="padding: 10px 20px; text-align: center;" id="action-cell-{{ $product->id }}">
                            @if (in_array($product->id, $pendingRestockRequestIds))
                                <span
                                    style="background: #eff6ff; color: #1d4ed8; padding: 6px 12px; border-radius: 20px; font-size: 12px; font-weight: 700; display: inline-flex; align-items: center; gap: 5px;">
