...
@section('content')
<div style="position: relative; min-height: calc(100vh - 115px); width: 100%;">
    ...
    <div id="enrollment-overlay">
        ...
        <button type="button" onclick="skipScanner()"
            style="position:absolute; top:30px; right:30px; background:#fff; color:var(--primary); border:2px solid var(--primary); padding:12px 25px; border-radius:12px; font-weight:800; cursor:pointer; transition:0.3s; display:flex; align-items:center; gap:10px; box-shadow:0 4px 15px rgba(0,77,153,0.15);">
            <i class="fa-solid fa-plus-circle"></i> AJOUTER SANS SCANNER
        </button>

        <div
            style="background:#fff; color:#333; padding:40px; border-radius:24px; max-width:500px; width:90%; box-shadow:0 10px 40px rgba(0,0,0,0.1); border:1px solid var(--border);">
            <div
                ...
                <i class="fa-solid fa-barcode" style="font-size:35px; color:var(--primary);"></i>
            </div>
            <h2 style="font-weight:800; font-size:24px; margin-bottom:10px;">Enrôlement Produit</h2>
            <p style="color:var(--muted); margin-bottom:30px;">Veuillez scanner le code-barres du nouveau produit pour commencer l'enregistrement.</p>

            <div id="scanner-animation"
                style="position:relative; width:200px; height:100px; border:2px dashed var(--primary); border-radius:12px; margin:0 auto 30px; overflow:hidden; background:#f8fafc; display:flex; align-items:center; justify-content:center;">
                <i class="fa-solid fa-keyboard" style="font-size:30px; color:#cbd5e1;"></i>
                <div id="scanner-line"
                    style="position:absolute; top:0; left:0; width:100%; height:2px; background:var(--primary); box-shadow:0 0 10px var(--primary); animation:scan 2s infinite ease-in-out;">
                </div>
            </div>

            <div style="display:flex; gap:10px; flex-direction:column;">
                <input type="text" id="manual-barcode" placeholder="Ou saisissez le code manuellement..."
                    style="width:100%; padding:15px; border:1px solid var(--border); border-radius:12px; outline:none; text-align:center; font-weight:700; font-size:18px;">
                <button type="button" onclick="confirmBarcode()"
                    style="background:var(--primary); color:white; border:none; padding:15px; border-radius:12px; font-weight:700; cursor:pointer; transition:0.3s; margin-top:10px;">CONFIRMER LE CODE</button>

                <a href="{{ route('magasinier.products.index') }}"
                    style="color:var(--muted); text-decoration:none; font-size:14px; margin-top:15px; display:inline-block;">Annuler et retourner</a>
            </div>
        </div>
    </div>
    ...
    <div class="prd-shell" id="product-form-shell" style="display:none;">
        <section class="prd-hero">
            <div>
                <p class="prd-kicker">MAGASINIER / PRODUITS</p>
                <h2>Ajouter un produit avec une fiche complète</h2>
                <p>Renseignez les informations de vente, stock et fournisseur pour une gestion fiable de la caisse et du magasin.</p>
            </div>
            <div class="prd-badge-box">
                <span class="prd-badge" id="stockBadge">Stock normal</span>
                <small id="stockHint">Le stock est supérieur au seuil d'alerte.</small>
            </div>
        </section>

        <form method="POST" action="{{ route('magasinier.products.store') }}" enctype="multipart/form-data" class="prd-grid"
            autocomplete="off">
            @csrf

            <article class="prd-card">
                <h3><i class="fa-solid fa-box"></i> Identité du produit</h3>
                ...
                <div class="prd-field">
                    ...
                    <div class="prd-input {{ $errors->has('name') ? 'is-invalid' : '' }}">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}" maxlength="255"
                            placeholder="Ex: Eau Minérale 1.5L" required>
                    </div>
                    @error('name')
                ...
                <div class="prd-field" style="display: none;">
                    ...
                    <small style="display:block; margin-top:6px; color:#5f7488; font-size:12px;">Ce code sera lié au produit pour le scan en caisse.</small>
                    @error('barcode_value')
                        <p class="prd-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="prd-row-2">
                    <div class="prd-field">
                        <label for="category_name">Catégorie <span>*</span></label>
                        <div class="prd-input {{ $errors->has('category_name') ? 'is-invalid' : '' }}">
                            <i class="fa-solid fa-layer-group"></i>
                            <select id="category_name" name="category_name" required>
                                <option value="" disabled {{ old('category_name') ? '' : 'selected' }}>Sélectionner</option>
                ...
                <div class="prd-field">
                    ...
                    <div class="prd-input prd-textarea {{ $errors->has('description') ? 'is-invalid' : '' }}">
                        ...
                        <textarea id="description" name="description" rows="4" placeholder="Détails ou caractéristiques du produit">{{ old('description') }}</textarea>
                    </div>
                    @error('description')
                        <p class="prd-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</p>
                    @enderror
            ...
            <article class="prd-card">
                <h3><i class="fa-solid fa-sack-dollar"></i> Prix, stock et média</h3>
                ...
                <div class="prd-field">
                    ...
                    <div class="prd-upload {{ $errors->has('image') ? 'is-invalid' : '' }}">
                        ...
                        <label for="image" class="prd-upload-btn"><i class="fa-solid fa-image"></i> Choisir une image</label>
                ...
                <div class="prd-preview" id="imagePreviewWrap">
                    ...
                    <img id="previewImage" alt="Aperçu produit">
                </div>

                <div class="prd-actions">
                    <a href="{{ route('magasinier.products.index') }}" class="prd-btn prd-btn-muted">
        ...
        <script>
            ...
            (function() {
                ...
                window.confirmBarcode = function() {
                    ...
                    fetch(`{{ route('magasinier.products.check-barcode') }}?barcode=${encodeURIComponent(code)}`)
            ...
            (function() {
                ...
                const syncStockState = () => {
                    ...
                    if (stock <= threshold) {
                        ...
                        stockHint.textContent = "Le stock est inférieur ou égal au seuil d'alerte.";
                    } else {
                        stockBadge.textContent = 'Stock normal';
                        stockBadge.style.background = '#dcfce7';
                        stockBadge.style.color = '#166534';
                        stockHint.textContent = "Le stock est supérieur au seuil d'alerte.";
                    }
                };
                ...
                const openBarcodeScanner = () => {
                    ...
                    Swal.fire({
                        ...
                        html: `
                            <div id="magasinier-scan-reader" style="width:100%; min-height:260px; border-radius:10px; overflow:hidden; background:#111;"></div>
                            <p id="magasinier-scan-status" style="margin-top:10px; color:#64748b; font-size:13px; text-align:center;">
                        ...
                        width: 560,
                        didOpen: async () => {
                            const statusEl = document.getElementById('magasinier-scan-status');

                            if (typeof Html5Qrcode !== 'undefined') {
                                try {
                                    html5Scanner = new Html5Qrcode('magasinier-scan-reader');
                            ...
                            const reader = document.getElementById('magasinier-scan-reader');
                            if (reader) {
                                reader.innerHTML = '';
                            ...
                            try {
                                ...
                                if (statusEl) {
                                    statusEl.innerHTML =
                                        '<span style="color:#e11d48"><i class="fa-solid fa-triangle-exclamation"></i> Impossible d\'ouvrir la caméra. Saisissez le code manuellement.</span>';
                                }
                            }
                        },
                        willClose: () => {
