...
@section('content')
    <div class="card" style="max-width:800px; padding:18px; margin: 0 auto;">
        <div style="display:flex; align-items:center; gap:12px; margin-bottom:20px; border-bottom:1px solid var(--border); padding-bottom:16px;">
            ...
            <div>
                <h2 style="font-size:18px; margin:0; font-weight:800; color:var(--primary);">Modifier la catégorie</h2>
                <p style="font-size:12px; color:var(--muted); margin:0;">Modifier les détails de la catégorie : <strong>{{ $category->name }}</strong></p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('magasinier.categories.update', $category) }}">
            ...
            <div style="margin-bottom:14px;">
                ...
                <input id="name" name="name" value="{{ old('name', $category->name) }}" required placeholder="Ex: Boissons, Épicerie..."
            ...
            <div style="margin-bottom:14px;">
                ...
                <textarea id="description" name="description" rows="4" placeholder="Description de la catégorie..."
                    style="width:100%; padding:10px; border:1px solid var(--border); border-radius:8px; outline:none; font-size:14px; font-family:inherit;">{{ old('description', $category->description) }}</textarea>
            </div>
            ...
            <div style="margin-bottom:14px; display:flex; align-items:center; gap:24px; flex-wrap:wrap;">
                <div>
                    ...
                    <input id="color" type="color" name="color" value="{{ old('color', $category->color ?? '#004d99') }}" 
            ...
            <div style="display:flex; justify-content:flex-end; gap:10px; border-top:1px solid var(--border); padding-top:16px; margin-top:20px;">
                ...
                <button class="mg-logout-btn" type="submit">
                    <i class="fa-solid fa-floppy-disk" style="margin-right:6px;"></i> Mettre à jour
                </button>
            </div>
        </form>
    </div>
